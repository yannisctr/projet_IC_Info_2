...
// Séparation des paramètres de requête en un tableau d'objets clé-valeur
...
lireCSVcongés("../Salariés/"+userId+'/congés.csv', function(joursConges) {
    bool1 =true
    verifAffichage();
});
...
function lireCSV_horaires(url, callback) {
    ...
    lireCSV_horaires("../Salariés/"+userId+'/horaires.csv', function(heure) {
        bool2 = true;
    verifAffichage();
});
...
lireCSV_absence("../Salariés/"+userId+'/Absences.csv', function(absence) {
    bool3 = true;
verifAffichage();
});
...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    buttons.forEach(button => {
        button.addEventListener('click', (event) => {
            ...
            envoyerMoisAuServeur(mois_actuel,annee,userId);
            afficheCalendrier();
        });
    });
